...
n_clusters <- unlist(lapply(graph_list, function(g) {
  length(cluster_infomap(g))
}))
...
gg_data <- data.frame(diagnosis_day,n_clusters, n_pathways)

# Plot
ggplot(gg_data, aes(x=diagnosis_day, y=n_clusters)) +
  geom_line()
