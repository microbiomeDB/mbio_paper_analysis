...
countries <- unique(sampleMetadata$country)


graph_list <- lapply(countries, function(x) {
  
  case_samples <- sampleMetadata$Sample_Id[sampleMetadata$country == x]
  ...
  case_correlation <- selfCorrelation(case_collection, method = 'pearson')
